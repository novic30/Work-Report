...
export function claimToken(claimToken: string) {
  return request<{
    token: string;
    user: { email: string; name: string; slug: string };
  }>(`/api/auth/claim/${claimToken}`);
}

export function getMe() {
  return request<{ user: { email: string; name: string; slug: string } }>(
    "/api/me",
  );
}
...
export function getEventTypes() {
  return request<{ eventTypes: import("../types").EventType[] }>(
    "/api/event-types",
  );
}

export function getEventType(id: number) {
  return request<{ eventType: import("../types").EventType }>(
    `/api/event-types/${id}`,
  );
}
...
export function getBookings(params?: {
  ...
  return request<{ bookings: import("../types").Booking[] }>(
...
export function getPublicClinic(clinicSlug: string) {
  return request<{
    clinic: { email: string; name: string; slug: string };
    eventTypes: Array<{
      id: number;
      name: string;
      slug: string;
