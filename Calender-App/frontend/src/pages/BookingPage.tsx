...
export function BookingPage() {
  const { clinicSlug, eventSlug } = useParams<{ clinicSlug: string; eventSlug: string }>();
      ...
      <div className="min-h-screen bg-gradient-to-br from-primary-50 to-white flex items-center justify-center">
        <div className="card max-w-lg text-center space-y-6 mx-6">
          ...
          <div className="rounded-lg bg-gray-50 p-4 text-left space-y-2">
            <p><strong className="text-gray-700">Event:</strong> {eventType.name}</p>
            <p><strong className="text-gray-700">Date:</strong> {selectedDate}</p>
            <p><strong className="text-gray-700">Time:</strong> {selectedTime}</p>
            <p><strong className="text-gray-700">Duration:</strong> {eventType.slot_duration_minutes} minutes</p>
            <p><strong className="text-gray-700">Name:</strong> {clientName}</p>
            <p><strong className="text-gray-700">Email:</strong> {clientEmail}</p>
          </div>
          ...
          {['date', 'slots', 'form'].map((s, i) => (
            <div key={s} className="flex items-center gap-2">
              <div className={`flex h-7 w-7 items-center justify-center rounded-full text-xs font-semibold ${
                ...
                ['date', 'slots', 'form'].indexOf(step) > i ? 'bg-primary-200 text-primary-700' :
          ...
          <div className="card space-y-4">
            ...
            {slotsLoading ? (
              ...
              <div className="text-center py-12 space-y-2">
                <p className="text-gray-500">{slots?.message || 'No available slots on this date.'}</p>
                <button
                  onClick={() => { setStep('date'); setSelectedDate(null); }}
                  className="text-sm text-primary-600 hover:text-primary-700"
                >
        ...
        {step === 'form' && (
          <div className="card space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => { setStep('slots'); setSelectedTime(null); }}
                className="text-sm text-primary-600 hover:text-primary-700"
              >
                ← Change time
              </button>
            ...
            <div className="rounded-lg bg-primary-50 border border-primary-200 p-3 text-sm text-primary-700">
              <strong>{eventType.name}</strong> · {selectedDate} at {selectedTime} · {eventType.slot_duration_minutes} min
            </div>
